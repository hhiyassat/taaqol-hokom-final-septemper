...
class NormalizationResult:
    ...
    #: مواضعُ في السطح المطبَّع أثبت فيها **حكمٌ مسمّى** غيابَ حرفٍ من البنية.
    #: انظر ``_Normalizer.elision_points`` — عقدٌ يقرؤه المحور الرابع.
    elision_points: list = field(default_factory=list)
...
class _Normalizer:
    def __init__(self, token: str, position: tuple | None, policy: OwnerPolicy,
        ...
        # القاعدة العامّة: يُوحَّد ترتيبُ العلامات عند الحدّ مرّةً واحدة، فلا
        # تبقى مقارنةٌ داخل المحرّك معتمدةً على ترتيب الطبع. والدالّة متساوية
        # القوّة، فتوحيدُ المحور صفر لا يُبطله تكرارُها هنا لمن يستدعي مباشرةً.
        token = canonical_mark_order(token)
        self.token = token
    ...
    # -- عقدُ الغياب المسمّى ----------------------------------------------
    @property
    def elision_points(self) -> list[int]:
        """مواضعُ القطع التي أثبت فيها حكمٌ مسمّى أن حرفًا غائبٌ من السطح.

        **القاعدة العامّة** — بديلُ ترقيعِ «بِالْقَلَمِ»:

        حدُّ المقطع مانعٌ للقشر لأنه بنيةُ الكلمة. لكنّ حدًّا **صنعه المحرّك
        بنفسه** حين حذف حرفًا ليس بنيةً بل أثرَ معالجة. فحين تحذف N7.2 همزةَ
        الوصل من «بِالْقَلَمِ» تلتصق الباءُ باللام الساكنة، فيصير أوّلُ مقطعٍ
        «بِلْ» = CVC، ويُحجَب القشرُ عن سابقةٍ **مرخّصة** لعلّةٍ من صنع
        المحرّك لا من صنع الكلمة.

        والقاعدة تُصاغ من مصدرٍ يملكه المحرّك أصلًا: **سجلّ المصائر**. فكلُّ
        موضعٍ حُذف فيه حرفٌ بحكمٍ مسمّى (N7.2 ، N9 ، …)، وكلُّ موضعٍ أثبت فيه
        حكمٌ أن حرفًا ساقطٌ من الرسم أصلًا (N7.4)، هو موضعُ قطعٍ مشروع.

        وحدُّها الذي يمنعها من أن تكون تخفيفًا عامًّا: لا تُولَّد نقطةٌ إلا من
        **حكمٍ مسمّى وقع فعلًا**. فـ«بِسْمِ» لا حذفَ فيها ولا إسقاط، فتبقى
        محجوبةً كما كانت — وشاهدُها الحيّ ``T1_BISMI_NOT_PEELED``.

        والحساب: كلُّ وحدةٍ في المخرج حرفٌ وعلامةٌ واحدة، فطولُها محرفان.
        """
        points: set[int] = set()
        for index, _char, fate, rule in self.res.fates:
            if fate == DELETED:
                before = sum(1 for u in self.units if u.source_index < index)
            elif rule == "N7.4" and fate == PRESERVED:
                # همزةُ «أل» ساقطةٌ من الرسم أصلًا: موضعُها بعد لام الجرّ.
                before = sum(1 for u in self.units if u.source_index <= index)
            else:
                continue
            points.add(2 * before)
    ...
    def run(self) -> NormalizationResult:
        ...
        self.res.elision_points = self.elision_points
        if not self.units:
...
def build_suite(policy: OwnerPolicy) -> CheckSuite:
    ...
    # كان هذا الفحص يدّعي فكَّ الشدّة في «اللَّهِ»، وكان **يمرّ لعلّةٍ خاطئة**:
    # سطحُه مكتوبٌ باليد بترتيبِ حركةٍ ثم شدّة، والنصّ يكتب الشدّة أوّلًا،
    # فلم يكن يطابق جردَ لفظ الجلالة فيسلك مسارَ التطبيع العامّ. ولمّا وُحّد
    # ترتيبُ العلامات عند الحدّ سلك مسارَه الصحيح: الاستثناء بالقاعدة ن٠-ج.
    # فالفحصُ الآن يثبت ما كان يخفيه.
    r = norm("اللَّهِ", (1, 1, 2))
    suite.check("T1_HAND_TYPED_JALALAH_IS_RECOGNISED",
                r.status == JALALAH and r.rules_applied == ["N0.J"],
                f"{r.status}  (توحيدُ ترتيب العلامات جعل المكتوبَ باليد كالمنقول)")

    # وخاصّيةُ فكّ الشدّة تُختبر حيث تقع فعلًا — في غير لفظ الجلالة.
    r = norm("الَّذِينَ", (2, 6, 2))
    suite.check("T1B_SHADDA_UNFOLDS_TO_TWO_UNITS", r.normalized == "ءَلْلَذِيْنَ",
                f"{r.normalized}  (ساكنٌ ثم متحرّك)")
