...
def _words_total():
    """`R3` — مقامُ الكلمات، مقيسًا من الجدول أو مُعلَنَ الغياب."""
    ...
    with p.open(encoding="utf-8", newline="") as fh:
        for r in _csv.DictReader(fh):
            n += sum(1 for c in (r.get("Owner_Decision_Classes") or "")
                     .split("|") if c.strip())
    return n
...
def measure() -> dict:
    ...
    m["residual_classes_words_total"] = _words_total()
...
def build_items(m: dict, g1: dict) -> list[Item]:
    ...
    # ── ر · بنودُ تحسين الإغلاق (`CLOSURE_IMPROVEMENT`) ───────────────
    # ولا واحدٌ منها يرفع علامةَ وثيقة النازلة خانةً واحدة. وهي ترفع دفترَ
    # الإصلاح وحدَه — والمقامان لا يُجمعان.
    add("R1", "سحبُ PEEL_LEDGER_CLOSES السابق صراحةً", "OWNED_BY_AGENT",
        "DONE",
        evidence={"withdrawn_claim": "PEEL_LEDGER_CLOSES = YES (74,668)",
                  "why": "قُوبل مقامان: تشغيلٌ بشاهدٍ وآخرُ بلا شاهد",
                  "superseded_by": "A7",
                  "effect_on_nazila_score": 0})
    add("R2", "مصفوفةُ انتقالٍ مغلقةٌ لأحكام B5", "OWNED_BY_AGENT", "DONE",
        evidence={"file": "output/closure/transition_b5.json",
                  "closes": True, "net": 0,
                  "denominator": m["axis4"]["words"],
                  "effect_on_nazila_score": 0})
    add("R3", "«--no-witness إنتاجيّ» — أمرٌ منشورٌ لا دعوى", "OWNED_BY_AGENT",
        "DONE",
        evidence={"command": "python3 -m aslot peel --no-witness",
                  "defined_in": "src/aslot/axes/axis4_peeling.py",
                  "claim_status": "TESTED_BY_PUBLISHED_COMMAND",
                  "not": "HYPOTHESIS",
                  "effect_on_nazila_score": 0})
    add("R4", "إحالةُ 134 إلى شاهدٍ مشتقٍّ باسمه وبصمته", "OWNED_BY_AGENT",
        "DONE",
        evidence={"files": m["operators"],
                  "numbers_previously_in_circulation":
                      [107, 160, 153, 102, 565],
                  "note": "134 رقمٌ سادس — والفصلُ بالاسم والبصمة لا بالعدد",
                  "effect_on_nazila_score": 0})
    ...
    return [
        ("B1", "أيُّ نصٍّ هو النازلة", {
            ...
            "note": "الخياران يعطيان بصمتَي مدخلٍ مختلفتين، فكلُّ تقريرٍ "
                    "لاحقٍ يتغيّر مقامُه"}),
        # `R3` — المجموعُ ١٨٬١٤٣ بمقام **الأحداث** لا الكلمات، وكان
        # يُسمّى `total_words_affected` فيُقرأ كلماتٍ متمايزة. ويُذكر
        # المقامان معًا: `DENOMINATOR_IS_PINNED`.
        ("B2", "أصنافُ البقيّة السبعة (T-4)",
         {"unassigned": m["residual_classes_unassigned"],
          "total_events_affected":
              sum(m["residual_classes_unassigned"].values()),
          "total_events_denominator":
              "مواضعُ حرفٍ يُرفع عندها صنف — لا كلماتٍ متمايزة",
          "total_words_affected": m.get("residual_classes_words_total"),
          "total_words_denominator":
              "كلماتٌ تحمل صنفًا — عمود Owner_Decision_Classes",
          "difference_accounted_in":
...
def closure(items: list[Item], g1: dict, findings: dict) -> dict:
    ...
    # خانةٌ عشريّةٌ واحدة في التقارير الثلاثة كلِّها. وتقريبٌ مختلفٌ في
    # تقريرين يجعل 58.6 تُقرأ 59 هنا و70.9 تُقرأ 71 هناك، فيُظنّ الفرقُ
    # قياسًا وهو تقريب.
    pot = round(100 * grounded / len(items), 1) if items else 0
    gates = {
