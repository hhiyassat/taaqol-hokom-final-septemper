...
* يُنفَّذ ببيئةٍ منفصلة (`.venv-taaqol`) لا يشاركها أسلوط، وإصداراها في
  `00_summary.json` لا هنا.
...
لا تميّز أصلًا، لا ضعيفًا». وذلك **مبالغةٌ سُحبت**: بنيتُها على نصوصٍ كلُّها
مشكولةٌ بالكامل (والإنجليزيّةُ لا مفتاحَ لها أصلًا)، فعمّمتُ نفيًا من شاهدٍ
ضيّق. وهو يميّز على السطح المجرَّد، ولا يميّز على المشكول — والعلّةُ في
مفاتيح الجرد لا في المصنِّف. والتصحيحُ للمالك، وقد أصاب.

**ولا رقمَ في هذا الشرح.** الأعدادُ في
`inspection/nazila/02_path_classifier.json` و`00_summary.json`، تُقرأ منهما.
وسببُ القيد واقعةٌ مسجَّلة: بقي هنا عددان من قياسٍ سابق بعد أن تغيّر
القياس، فقال الشرحُ غيرَ ما يقيس الملفّ. والرقمُ في نثرٍ لا يُعاد حسابُه
يموت صامتًا.
"""
...
def to_plain(value):
    ...
    **وهويّةُ التعداد ليست قيمتَه دائمًا.** كان هذا يأخذ `.value` لكلّ تعداد،
    ...
    والقاعدةُ العامّة، لا ترقيعُ الرتبة وحدَها:
        ...
        تعدادٌ قيمتُه عدد    ← القيمةُ **لا تُسمّي**، فتُؤخذ `.name`
        تعدادٌ قيمتُه نصّ    ← القيمةُ اسمٌ صالح، فتُؤخذ `.value`

    والعلّةُ **أنّ العدد لا يُسمّي**، لا أنّه ترتيب. وتعليلُها بالترتيب يبني
    قاعدةً أوسعَ من دليلها: رمزٌ رقميٌّ لا ترتيبَ فيه يخرج باسمه كذلك، وهو
    صوابٌ بالعلّة الصحيحة لا بالأولى.
