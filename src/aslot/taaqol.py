...
OWNER_DECISIONS_PENDING: tuple[str, ...] = (
    ...
    # **حُسم شطرُه.** حكم المالك: «عالج الفاصلة والنقطة». فصارت `،` و`.`
    # فاصلتين تُقطعان من طرفَي التوكن ويُسجَّل ما قُطع — ٢٨ توكنًا عادت إلى
    # الجرد، وصفرٌ تغيّر في المصحف. انظر `strip_owner_separators`.
    #
    # والباقي **لم يُحكم فيه**، فلا يُمدّ إليه الحكمُ قياسًا: `؟` و`؛` و`:`
    # ما زالت تُخرج التوكنَ كلَّه (٧ توكنات في المثبَّتات). ومدُّ حكمٍ إلى ما
    # لم يشمله هو الخرقُ الذي لا يشتكي منه أحد، فيُرفع بدلًا من أن يُفترض.
    "OPEN:QUESTION_SEMICOLON_COLON_SEPARATOR_OR_LETTER",
