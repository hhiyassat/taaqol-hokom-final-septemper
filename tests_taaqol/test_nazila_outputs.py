...
def test_the_page_has_fourteen_sections_and_a_complete_matrix(page):
    """كانت اثنَي عشرَ فصلًا، وصارت أربعةَ عشرَ بسببَين مُعلنَين.

    زِيد فصلُ **حدّ الدخول** أوّلًا — لأنّ غيابَه يجعل القارئَ يظنّ ما يراه
    تحليلًا ناقصًا، وهو ليس تحليلًا أصلًا — وزِيد **الدفترُ كاملًا** آخِرًا،
    وبه صار `G5` بنيويًّا. والعددُ يُثبَّت بعدَه كي لا يتحرّك صامتًا.
    """
    assert len(re.findall(r"<h2>", page)) == 14
    assert page.index("حدُّ الدخول المعلَن") < page.index("الهُويّة والبصمة")
    assert len(re.findall(r'<td class="s-', page)) == 160


def test_the_entry_boundary_section_carries_its_seven_cells(page, cells):
    """سبعُ خاناتٍ، ولا واحدةَ منها مُنشأة — و`EntryBoundary` لا تُستورد."""
    seven = [c for c in cells if c["section"].startswith("0.")]
    assert len(seven) == 7
    assert {c["reason"] for c in seven} == {"NOT_CONSTRUCTED_IN_SOURCE"}
    for c in seven:
        assert f'data-field="{c["field"]}"' in page
    assert "شكلٌ مقيس، لا تحليل" in page


def test_every_cell_has_a_row_in_the_page(page, cells):
    """`G5` مقيسًا من الصفحة المكتوبة: لا خانةَ بلا صفّ."""
    pairs = set(re.findall(r'data-section="([^"]*)" data-field="([^"]*)"',
                           page))
    import html as _h
    pairs = {(_h.unescape(a), _h.unescape(b)) for a, b in pairs}
    missing = [f'{c["section"]}/{c["field"]}' for c in cells
               if (c["section"], c["field"]) not in pairs]
...
# ── المعيارُ السادس · يُعاد الإنتاجُ بلا فرق ───────────────────────────
def test_the_run_reproduces_and_the_only_variance_is_named(scores, tmp_path):
    """٣٬٨٤٠ خانةً بلا فرق — ويُثبت ذلك بتشغيلٍ ثانٍ، لا بدعوى.

    وأوّلُ مقابلةٍ أظهرت أنّ `cells.csv` يختلف بين تشغيلَين. والعلّةُ خانةٌ
    واحدة: `utc_timestamp` — مقيسةٌ لا عشوائيّة. فتُستثنى **بالاسم** في
    `content_fingerprint`، لا بالصمت عنها.
    """
    import subprocess
    import sys
    dest = tmp_path / "again"
    # المفسِّرُ هو الذي يُشغَّل الآن — لا بيئةٌ مسمّاةٌ باسمها (`B11`).
    r = subprocess.run(
        [sys.executable, str(GEN), "--out", str(dest)],
        capture_output=True, text=True, cwd=ROOT, check=False)
    assert r.returncode == 0, r.stdout + r.stderr
    again = json.loads((dest / "scores.json").read_text(encoding="utf-8"))

    a, b = scores["content_fingerprint"], again["content_fingerprint"]
    assert a["records_csv_sha256"] == b["records_csv_sha256"]
    assert a["cells_csv_sha256_excluding"] == b["cells_csv_sha256_excluding"]
    assert a["excluded_fields"] == ["utc_timestamp"]
    assert again["GROUNDED"] == scores["GROUNDED"] == 207
    assert again["CELLS_TOTAL"] == scores["CELLS_TOTAL"] == 292

    with (dest / "records.csv").open(encoding="utf-8", newline="") as fh:
        rows = list(csv.reader(fh))[1:]
    assert sum(len(x) for x in rows) == 3840 == 160 * 24


def test_the_excluded_field_is_a_single_named_cell(cells):
    """والمستثنى خانةٌ واحدةٌ بعينها — لا بابٌ يُوسَّع كلّما اختلف تشغيل."""
    volatile = [c for c in cells if c["field"] == "utc_timestamp"]
    assert len(volatile) == 1
    assert volatile[0]["source"] == "datetime"
